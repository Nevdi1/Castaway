(() => {
  "use strict";
  let t = (t, e = 500, s = 0) => {
      t.classList.contains("_slide") ||
        (t.classList.add("_slide"),
        (t.style.transitionProperty = "height, margin, padding"),
        (t.style.transitionDuration = e + "ms"),
        (t.style.height = `${t.offsetHeight}px`),
        t.offsetHeight,
        (t.style.overflow = "hidden"),
        (t.style.height = s ? `${s}px` : "0px"),
        (t.style.paddingTop = 0),
        (t.style.paddingBottom = 0),
        (t.style.marginTop = 0),
        (t.style.marginBottom = 0),
        window.setTimeout(() => {
          ((t.hidden = !s),
            !s && t.style.removeProperty("height"),
            t.style.removeProperty("padding-top"),
            t.style.removeProperty("padding-bottom"),
            t.style.removeProperty("margin-top"),
            t.style.removeProperty("margin-bottom"),
            !s && t.style.removeProperty("overflow"),
            t.style.removeProperty("transition-duration"),
            t.style.removeProperty("transition-property"),
            t.classList.remove("_slide"));
        }, e));
    },
    e = (t, e = 500, s = 0) => {
      if (!t.classList.contains("_slide")) {
        (t.classList.add("_slide"),
          (t.hidden = !t.hidden && null),
          s && t.style.removeProperty("height"));
        let a = t.offsetHeight;
        ((t.style.overflow = "hidden"),
          (t.style.height = s ? `${s}px` : "0px"),
          (t.style.paddingTop = 0),
          (t.style.paddingBottom = 0),
          (t.style.marginTop = 0),
          (t.style.marginBottom = 0),
          t.offsetHeight,
          (t.style.transitionProperty = "height, margin, padding"),
          (t.style.transitionDuration = e + "ms"),
          (t.style.height = a + "px"),
          t.style.removeProperty("padding-top"),
          t.style.removeProperty("padding-bottom"),
          t.style.removeProperty("margin-top"),
          t.style.removeProperty("margin-bottom"),
          window.setTimeout(() => {
            (t.style.removeProperty("height"),
              t.style.removeProperty("overflow"),
              t.style.removeProperty("transition-duration"),
              t.style.removeProperty("transition-property"),
              t.classList.remove("_slide"));
          }, e));
      }
    },
    s = !0,
    a = (t = 500) => {
      let e = document.querySelector("body");
      if (s) {
        let a = document.querySelectorAll("[data-lp]");
        (setTimeout(() => {
          for (let t = 0; t < a.length; t++) {
            a[t].style.paddingRight = "0px";
          }
          ((e.style.paddingRight = "0px"),
            document.documentElement.classList.remove("lock"));
        }, t),
          (s = !1),
          setTimeout(function () {
            s = !0;
          }, t));
      }
    },
    o = (t = 500) => {
      let e = document.querySelector("body");
      if (s) {
        let a = document.querySelectorAll("[data-lp]");
        for (let t = 0; t < a.length; t++) {
          a[t].style.paddingRight =
            window.innerWidth -
            document.querySelector(".wrapper").offsetWidth +
            "px";
        }
        ((e.style.paddingRight =
          window.innerWidth -
          document.querySelector(".wrapper").offsetWidth +
          "px"),
          document.documentElement.classList.add("lock"),
          (s = !1),
          setTimeout(function () {
            s = !0;
          }, t));
      }
    };
  function i(t, e) {
    const s = Array.from(t).filter(function (t, s, a) {
      if (t.dataset[e]) return t.dataset[e].split(",")[0];
    });
    if (s.length) {
      const t = [];
      s.forEach((s) => {
        const a = {},
          o = s.dataset[e].split(",");
        ((a.value = o[0]),
          (a.type = o[1] ? o[1].trim() : "max"),
          (a.item = s),
          t.push(a));
      });
      let a = t.map(function (t) {
        return (
          "(" + t.type + "-width: " + t.value + "px)," + t.value + "," + t.type
        );
      });
      a = (function (t) {
        return t.filter(function (t, e, s) {
          return s.indexOf(t) === e;
        });
      })(a);
      const o = [];
      if (a.length)
        return (
          a.forEach((e) => {
            const s = e.split(","),
              a = s[1],
              i = s[2],
              r = window.matchMedia(s[0]),
              n = t.filter(function (t) {
                if (t.value === a && t.type === i) return !0;
              });
            o.push({ itemsArray: n, matchMedia: r });
          }),
          o
        );
    }
  }
  let r = !1;
  (setTimeout(() => {
    if (r) {
      let t = new Event("windowScroll");
      window.addEventListener("scroll", function (e) {
        document.dispatchEvent(t);
      });
    }
  }, 0),
    (window.FLS = !0),
    (function (t) {
      let e = new Image();
      ((e.onload = e.onerror =
        function () {
          t(2 == e.height);
        }),
        (e.src =
          "data:image/webp;base64,UklGRjoAAABXRUJQVlA4IC4AAACyAgCdASoCAAIALmk0mk0iIiIiIgBoSygABc6WWgAA/veff/0PP8bA//LwYAAA"));
    })(function (t) {
      let e = !0 === t ? "webp" : "no-webp";
      document.documentElement.classList.add(e);
    }),
    (function () {
      let t = document.querySelector(".icon-menu");
      t &&
        t.addEventListener("click", function (t) {
          s &&
            (((t = 500) => {
              document.documentElement.classList.contains("lock") ? a(t) : o(t);
            })(),
            document.documentElement.classList.toggle("menu-open"));
        });
    })(),
    (function () {
      const s = document.querySelectorAll("[data-tabs]");
      let a = [];
      if (s.length > 0) {
        const t = location.hash.replace("#", "");
        (t.startsWith("tab-") && (a = t.replace("tab-", "").split("-")),
          s.forEach((t, e) => {
            (t.classList.add("_tab-init"),
              t.setAttribute("data-tabs-index", e),
              t.addEventListener("click", r),
              (function (t) {
                const e = t.querySelectorAll("[data-tabs-titles]>*"),
                  s = t.querySelectorAll("[data-tabs-body]>*"),
                  o = t.dataset.tabsIndex,
                  i = a[0] == o;
                if (i) {
                  t.querySelector(
                    "[data-tabs-titles]>._tab-active",
                  ).classList.remove("_tab-active");
                }
                s.length > 0 &&
                  s.forEach((t, s) => {
                    (e[s].setAttribute("data-tabs-title", ""),
                      t.setAttribute("data-tabs-item", ""),
                      i && s == a[1] && e[s].classList.add("_tab-active"),
                      (t.hidden = !e[s].classList.contains("_tab-active")));
                  });
              })(t));
          }));
        let e = i(s, "tabs");
        e &&
          e.length &&
          e.forEach((t) => {
            (t.matchMedia.addEventListener("change", function () {
              o(t.itemsArray, t.matchMedia);
            }),
              o(t.itemsArray, t.matchMedia));
          });
      }
      function o(t, e) {
        t.forEach((t) => {
          const s = (t = t.item).querySelector("[data-tabs-titles]"),
            a = t.querySelectorAll("[data-tabs-title]"),
            o = t.querySelector("[data-tabs-body]");
          t.querySelectorAll("[data-tabs-item]").forEach((i, r) => {
            e.matches
              ? (o.append(a[r]), o.append(i), t.classList.add("_tab-spoller"))
              : (s.append(a[r]), t.classList.remove("_tab-spoller"));
          });
        });
      }
      function r(s) {
        const a = s.target;
        if (a.closest("[data-tabs-title]")) {
          const o = a.closest("[data-tabs-title]"),
            i = o.closest("[data-tabs]");
          if (
            !o.classList.contains("_tab-active") &&
            !i.querySelectorAll("._slide").length
          ) {
            const s = i.querySelector("[data-tabs-title]._tab-active");
            (s && s.classList.remove("_tab-active"),
              o.classList.add("_tab-active"),
              (function (s) {
                const a = s.querySelectorAll("[data-tabs-title]"),
                  o = s.querySelectorAll("[data-tabs-item]"),
                  i = s.dataset.tabsIndex,
                  r = (function (t) {
                    if (t.hasAttribute("data-tabs-animate"))
                      return t.dataset.tabsAnimate > 0
                        ? t.dataset.tabsAnimate
                        : 500;
                  })(s);
                o.length > 0 &&
                  o.forEach((s, o) => {
                    a[o].classList.contains("_tab-active")
                      ? (r ? e(s, r) : (s.hidden = !1),
                        s.closest(".popup") ||
                          (location.hash = `tab-${i}-${o}`))
                      : r
                        ? t(s, r)
                        : (s.hidden = !0);
                  });
              })(i));
          }
          s.preventDefault();
        }
      }
    })(),
    (function () {
      r = !0;
      const t = document.querySelector("header.header"),
        e = t.hasAttribute("data-scroll-show"),
        s = t.dataset.scrollShow ? t.dataset.scrollShow : 500,
        a = t.dataset.scroll ? t.dataset.scroll : 1;
      let o,
        i = 0;
      document.addEventListener("windowScroll", function (r) {
        const n = window.scrollY;
        (clearTimeout(o),
          n >= a
            ? (!t.classList.contains("_header-scroll") &&
                t.classList.add("_header-scroll"),
              e &&
                (n > i
                  ? t.classList.contains("_header-show") &&
                    t.classList.remove("_header-show")
                  : !t.classList.contains("_header-show") &&
                    t.classList.add("_header-show"),
                (o = setTimeout(() => {
                  !t.classList.contains("_header-show") &&
                    t.classList.add("_header-show");
                }, s))))
            : (t.classList.contains("_header-scroll") &&
                t.classList.remove("_header-scroll"),
              e &&
                t.classList.contains("_header-show") &&
                t.classList.remove("_header-show")),
          (i = n <= 0 ? 0 : n));
      });
    })());
  new Swiper(".swiper", {
    loop: !0,
    slidesPerView: 3,
    speed: 700,
    grid: { rows: 2, fill: "row" },
    spaceBetween: 16,
    breakpoints: {
      320: { slidesPerView: 1, grid: { rows: 2 } },
      768: { slidesPerView: 2, grid: { rows: 2 } },
      1024: { slidesPerView: 3, grid: { rows: 2 } },
    },
    navigation: {
      nextEl: ".swiper-button-next",
      prevEl: ".swiper-button-prev",
    },
  });
})();
